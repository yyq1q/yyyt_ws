...
communicatortest_spec = ["implementation_id", "CommunicatorTest", 
...
# 
# </rtc-template>
class CommunicatorTest(OpenRTM_aist.DataFlowComponentBase):
...
def RunTest():
    ...
    comp = manager.getComponent("CommunicatorTest0")
    if comp is None:
        print('Component get failed.', file=sys.stderr)
        return False
    return comp.runTest()

def CommunicatorTestInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str=communicatortest_spec)
    manager.registerFactory(profile,
                            CommunicatorTest,
                            OpenRTM_aist.Delete)

def MyModuleInit(manager):
    CommunicatorTestInit(manager)
    Communicator.CommunicatorInit(manager)

    # Create a component
    comp = manager.createComponent("CommunicatorTest")
